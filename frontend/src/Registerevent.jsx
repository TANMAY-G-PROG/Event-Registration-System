...
const SLIDER_CONSTANTS = {
  AXIS_LOCK_THRESHOLD: 10,       // px before committing to an axis
  AXIS_DOMINANCE_RATIO: 1.8,     // deltaX must be 1.8x deltaY to count as horizontal (wheel)
...
// DRAG HOOK
//
// FIX 1 — preventDefault on the SAME frame axis locks to 'x'
//   Old code only called preventDefault on events AFTER the lock was set,
//   meaning the browser still prepared a vertical scroll for 1-2 frames.
//   Now we call preventDefault immediately in the same event that commits the lock.
//
// FIX 2 — CSS touch-action flipped via ref
//   Slider element starts with touch-action:'pan-y' (browser handles vertical scroll).
//   The moment we lock to 'x' we switch to touch-action:'none' so the browser
//   stops ALL its own scrolling logic and we own the gesture entirely.
//   On drag end / cancel we restore 'pan-y'.
// ============================================================================

function useSliderDrag({ sliderRef, trackRef, currentIndex, snapPoints, onSwipeLeft, onSwipeRight }) {
  ...
  // Stable refs so DOM listeners don't need re-registering on every render
  ...
  // Helper: set touch-action on the slider element directly
  const setTouchAction = useCallback((value) => {
    if (sliderRef.current) sliderRef.current.style.touchAction = value;
  }, [sliderRef]);
  ...
  const handleStart = useCallback((clientX, clientY) => {
    ...
    setIsDragging(true);
    // Start with pan-y so browser can still handle vertical scroll
    // until we confirm the gesture is horizontal
  ...
  const handleMove = useCallback((clientX, clientY, event) => {
    ...
    if (directionLockedRef.current === null) {
      ...
      if (directionLockedRef.current === 'x') {
        // FIX: preventDefault and touch-action:'none' on the SAME frame we lock.
        // This stops vertical scroll from bleeding through for those first 1-2 frames.
        setTouchAction('none');
        if (event) { try { event.preventDefault(); } catch (e) {} }
      } else {
        // Vertical — restore pan-y, cancel drag, let browser scroll
        setTouchAction('pan-y');
    ...
    // Horizontal — prevent any remaining page scroll
    if (event) { try { event.preventDefault(); } catch (e) {} }
  ...
  const handleEnd = useCallback(() => {
    ...
    setIsDragging(false);
    if (rafRef.current) cancelAnimationFrame(rafRef.current);
    // Always restore pan-y so vertical scroll works again
    setTouchAction('pan-y');
  ...
  // Mouse handlers (desktop) via React props — fine to be passive
  ...
  // Touch handlers — attached directly as { passive: false } so preventDefault works
  useEffect(() => {
    const el = sliderRef.current;
    if (!el) return;

    // Set initial touch-action so vertical scroll works when not swiping
  ...
  }, [sliderRef, handleStart, handleMove, handleEnd]);

  return { isDragging, handleMouseDown, handleMouseMove, handleMouseUp, handleMouseLeave };
}

// ============================================================================
// WHEEL HOOK
//
// FIX 3 — Axis dominance check for Windows trackpad
//   Windows sends deltaX and deltaY simultaneously during diagonal gestures.
//   Old code picked whichever was bigger — but on near-diagonal swipes they
//   alternated every event, causing left-right-left jitter.
//   Now: deltaX must be AXIS_DOMINANCE_RATIO (1.8x) larger than deltaY to
//   count as a horizontal swipe. Ambiguous diagonals are ignored entirely.
// ============================================================================
...
function useSliderWheel({ sliderRef, onScrollLeft, onScrollRight, enabled }) {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      ...
      // Ignore micro-events (trackpad noise)
      if (absDx < SLIDER_CONSTANTS.WHEEL_MIN_DELTA && absDy < SLIDER_CONSTANTS.WHEEL_MIN_DELTA) return;

      // FIX: Require clear horizontal dominance.
      // If deltaX is not at least 1.8x deltaY, the gesture is too diagonal — skip it.
      // This stops Windows trackpad diagonal events from triggering the slider.
      const isHorizontallyDominant = absDx > absDy * SLIDER_CONSTANTS.AXIS_DOMINANCE_RATIO;
      // Also allow pure horizontal events with no vertical component
      const isPureHorizontal = absDx > SLIDER_CONSTANTS.WHEEL_MIN_DELTA && absDy < 3;

      if (!isHorizontallyDominant && !isPureHorizontal) return;

      // Cooldown: one card change per gesture
      if (cooldownRef.current) return;
      cooldownRef.current = true;

      if (e.deltaX > 0) onScrollLeft();
      else onScrollRight();

      if (cooldownTimerRef.current) clearTimeout(cooldownTimerRef.current);
      cooldownTimerRef.current = setTimeout(() => {
        cooldownRef.current = false;
      }, SLIDER_CONSTANTS.WHEEL_DEBOUNCE_MS);
    };
  ...
  useEffect(() => {
    function measure() {
      ...
      const points = cards.map(card => {
        ...
        return viewportCentreX - (rect.left + rect.width / 2);
      });
      setSnapPoints(points);
      if (cards.length >= 1) setCardWidth(cards[0].getBoundingClientRect().width);
      if (cards.length >= 2) { const r0 = cards[0].getBoundingClientRect(); const r1 = cards[1].getBoundingClientRect(); setCardGap(r1.left - r0.right); }
...
function NavigationDots({ total, current, onSelect }) {
  ...
  return (
    <div className="re-gallery-dots">
      {showLeftEllipsis && <button className="re-gallery-dot re-gallery-dot-ellipsis" onClick={() => onSelect(0)} title="Go to first" />}
      {visibleDots.map((index) => (
        <button key={index} onClick={() => onSelect(index)}
          className={`re-gallery-dot ${index === current ? 'active' : ''}`}
          style={{ backgroundColor: index === current ? 'var(--nb-black)' : 'rgba(10,10,10,0.2)', width: index === current ? '28px' : '8px' }}
          aria-label={`Go to event ${index + 1}`} />
      ))}
      {showRightEllipsis && <button className="re-gallery-dot re-gallery-dot-ellipsis" onClick={() => onSelect(total - 1)} title="Go to last" />}
    </div>
  );
}
...
function JumpCounter({ current, total, onJump }) {
  ...
  const handleActivate = () => { setEditing(true); setInputVal(""); setTimeout(() => inputRef.current?.select(), 0); };
  const handleCommit = () => {
  ...
  return (
    <div className={`re-gallery-counter ${editing ? 'editing' : ''}`} onClick={!editing ? handleActivate : undefined} title={editing ? "" : "Click to jump to event"}>
      {editing ? (
...
function SearchBar({ events, onSelect, currentIndex }) {
  ...
  const handleKeyDown = (e) => { if (e.key === "Escape") { setQuery(""); setFocused(false); inputRef.current?.blur(); } e.stopPropagation(); };
  return (
    <div ref={containerRef} className={`re-search-wrap ${focused ? 'expanded' : ''}`}>
      <button className="re-search-icon-btn" onClick={() => { setFocused(true); setTimeout(() => inputRef.current?.focus(), 50); }} tabIndex={focused ? -1 : 0} aria-label="Search events">
      ...
      <input ref={inputRef} className="re-search-inline-input" placeholder="Search…" value={query} onChange={e => setQuery(e.target.value)} onFocus={() => setFocused(true)} onBlur={() => setTimeout(() => { setFocused(false); setQuery(""); }, 160)} onKeyDown={handleKeyDown} autoComplete="off" tabIndex={focused ? 0 : -1} />
...
function BentoGrid({ events, onOpen, renderControls }) {
  return (
    <div className="re-bento-grid-container"><div className="re-bento-grid">
      {events.map(event => <BentoGridCard key={event.eid} event={event} onOpen={onOpen} renderControls={renderControls} />)}
    </div></div>
  );
}
    ...
    <div className={`re-gallery-card ${isActive ? 'active' : ''}`} style={{ transform: `scale(${scale}) translateZ(0)`, opacity, WebkitTransform: `scale(${scale}) translateZ(0)` }}>
      <div className="re-gallery-card-frame">
        <div className="re-gallery-image-container">
          <img src={resolveBanner(event)} alt={event.ename} style={{ transform: 'scale(1)' }} draggable={false} loading="lazy" />
...
export default function Registerevent() {
  ...
  const fetchMyRegistrations = useCallback(async () => {
    try {
      ...
      if (res.ok) { const data = await res.json(); setRegisteredEvents(new Set(data.participantEvents.map(ev => ev.eid))) }
  ...
  const loadTeamStatus = useCallback(async (eventId) => {
    try {
      ...
      if (response.ok) { const data = await response.json(); setTeamStates(prev => ({ ...prev, [eventId]: data })) }
  ...
  const { isDragging, handleMouseDown, handleMouseMove, handleMouseUp, handleMouseLeave } = useSliderDrag({
    sliderRef, trackRef, currentIndex, snapPoints,
  ...
  useSliderWheel({ sliderRef, onScrollLeft: goToNext, onScrollRight: goToPrev, enabled: wheelEnabled });
  ...
  async function handleCreateTeam(eventId) {
    try {
      ...
      const response = await apiFetch(`/api/events/${eventId}/create-team`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ teamName: teamName.trim(), memberUSNs: validUSNs }) });
      const data = await response.json()
  ...
  async function handleSubmitUpiPayment() {
    ...
    try {
      const response = await apiFetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ transaction_id: transactionId.trim() }) });
      const data = await response.json()
      if (!response.ok) { showModalFlash('error', data.error); return }
  ...
  function renderControls(event, isOverlay = false) {
    ...
    if (teamState.registrationComplete) return (<div className="registerevent-btn-group"><button className="registerevent-btn success" disabled>✓ Team Registered</button>{aboutBtn}</div>)
      ...
      return (
        <div className="registerevent-team-controls-group" style={{ width: '100%' }}>
          {isOverlay && (
            <div className="registerevent-hud-panel">
              <div className="registerevent-hud-header">
                ...
                <span className="registerevent-hud-value" style={{ color: teamState.canRegister ? '#00ff9d' : '#ffbd00' }}>{teamState.joinedCount}/{teamState.minSize} Members</span>
              </div>
              <div className="registerevent-member-stack">
                ...
                {teamState.members?.map((member, idx) => (
                  <div key={idx} className="registerevent-member-row">
                    ...
                    <span className={`registerevent-status-indicator ${member.join_status ? "joined" : "pending"}`}>{member.join_status ? "Accepted" : "Pending"}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="registerevent-btn-group" style={{ marginTop: isOverlay ? '16px' : '0' }}>
            {isLeader ? (
              <button className={`registerevent-btn ${teamState.canRegister ? "primary" : "disabled"}`} onClick={(e) => { e.stopPropagation(); teamState.canRegister && handleRegisterTeam(event, teamState); }} disabled={!teamState.canRegister}>
    ...
    <main className="registerevent-page">
        ...
        <div className="re-gallery-header-right">
          ...
          {filteredEvents.length > 0 && viewMode === "grid" && (
            <div className="re-gallery-counter" style={{ cursor: 'default' }}>
              <span>{filteredEvents.length}</span><span style={{ color: 'rgba(255,255,255,0.25)', margin: '0 3px' }}>·</span><span>events</span>
            </div>
          )}
        </div>
      </header>
      ...
      {loading ? (
        ...
        <div
          ...
          // NOTE: touch handlers attached via useEffect as {passive:false} — do NOT add onTouchStart/Move here
        >
      ...
      {!loading && filteredEvents.length > 1 && viewMode === "gallery" && (
        <NavigationDots total={filteredEvents.length} current={currentIndex} onSelect={goToSlide} />
      )}
      {viewMode === "gallery" && (
        <div className="re-gallery-keyboard-hint"><kbd>←</kbd><kbd>→</kbd><span>navigate</span></div>
      )}
                  ...
                  <div className="registerevent-form-group">
                    ...
                    {teamFormData.memberUSNs.map((usn, i) => (
                      <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                        <input className="registerevent-form-input" placeholder="Member USN" value={usn} onChange={e => { const newUsns = [...teamFormData.memberUSNs]; newUsns[i] = e.target.value; setTeamFormData({ ...teamFormData, memberUSNs: newUsns }) }} />
                        {i > 0 && <button className="registerevent-team-action-btn" style={{ background: 'var(--nb-red)', color: '#fff', borderColor: 'var(--nb-black)', padding: '0 10px', width: 'auto' }} onClick={() => { const newUsns = teamFormData.memberUSNs.filter((_, idx) => idx !== i); setTeamFormData({ ...teamFormData, memberUSNs: newUsns }); }}>×</button>}
                      </div>
                    ))}
